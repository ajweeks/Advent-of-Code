...
pub const String = struct {
    ...
    /// Creates a String with an Allocator and a given initial contents
    /// User is responsible for managing the new String
    pub fn new(allocator: std.mem.Allocator, contents: []const u8) Error!String {
        var result = String.init(allocator);
        try result.insert(contents, 0);
        return result;
    }
